...
### Optional: OpenWebUI front-end

If you prefer a chat UI, an optional [OpenWebUI](https://github.com/open-webui/open-webui) container is wired to the launcher. Start it with the dedicated profile:

```bash
docker compose --profile launcher --profile ui up -d openwebui
```

The UI listens on [http://localhost:3000](http://localhost:3000) and talks to the launcher using the same `VLLM_API_KEY` you configure for API clients. Sign in with the default admin account that OpenWebUI prompts for on first launch and add the models exposed by the launcher (no extra configuration needed—the container already points at `http://launcher:8000`).
